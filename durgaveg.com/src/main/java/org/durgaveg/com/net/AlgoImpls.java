...
public class AlgoImpls extends SparkParent{
	...
	private Dataset<Row> runStaticVersion() {
		 Dataset<Row> staticRows = 
				session
				.read()
				.format("csv")
	...
	private void runQuery(Dataset<Row> rows,boolean stream) {
		...
		rows
		.select(new Column("CustomerID"),
				new Column("InvoiceDate"),
				new Column("Quantity"),
				new Column("UnitPrice"),
				functions.expr("Quantity * UnitPrice").as("UnitTotal"))
		.groupBy(
				new Column("CustomerID"),
				functions.window(new Column("InvoiceDate"),"1 day" ))
		.sum("UnitTotal")
		;
		if(!stream)
		queryResult.show();
	...
	public void init() {
		Dataset<Row> rawRows = runStaticVersion();
	...
	public Dataset<Row> transformToVector(Dataset<Row> rows,String col){
		...
		.withColumn("DayOfWeek", functions.date_format(new Column("InvoiceDate"), "EEEE"));
