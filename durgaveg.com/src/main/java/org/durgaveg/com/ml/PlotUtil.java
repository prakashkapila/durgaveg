...
public class PlotUtil {
    ...
    private static XYDataset createDataSetTrain(INDArray features, INDArray labels ){
        ...
        INDArray argMax = Nd4j.getExecutioner().exec(new IMax(labels));
        for( int i=0; i<nRows; i++ ){
    ...
    private static XYDataset createDataSetTest(INDArray features, INDArray labels, INDArray predicted ){
        ...
        INDArray actualIdx = Nd4j.getExecutioner().exec(new IMax(labels));
        INDArray predictedIdx = Nd4j.getExecutioner().exec(new IMax(predicted));
        for( int i=0; i<nRows; i++ ){
